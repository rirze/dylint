...
fn one_name_multiple_toolchains() {
    ...
    .unwrap();
    let _: PathBuf =
        debug_path_with_toolchain(tempdir.path(), "fill_me_in", msrv::MSRV_CHANNEL).unwrap();
    ...
    .unwrap();
    let _: PathBuf =
        debug_path_with_toolchain(tempdir.path(), "fill_me_in", msrv::MSRV_PLUS_1_CHANNEL).unwrap();
